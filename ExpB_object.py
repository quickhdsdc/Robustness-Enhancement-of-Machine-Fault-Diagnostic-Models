    ...
    for i in range(len(speeds)):
        if baseline == 'ResNet':
            ...
            if health == 'Good':
                ...
                bad_speed = np.load('./Data/All_featurespace/' + varname_bad + '.npy')                   
        if health == 'Good':
            good.append(good_speed[:499])
        else:
            bad.append(bad_speed[:499])
    if health == 'Good':    
        x=np.vstack(good)
        n = len(x)
...
baselines = ['CWT+ResNet','ResNet']
sample_size = 10240
repeats = 10  
rs_init = 1

acc_GBDT = np.zeros((10,4))
acc_ResNet = np.zeros((10,4))
acc_HT = np.zeros((10,4))
acc_CWT = np.zeros((10,4))
for baseline in baselines:
    ...
    for n in range(repeats):
        ...
        x_test_2, y_test_2 = readdata(speeds,'Bad',dataset_test_2,baseline)       
